% plot ground speed measuring system data
...
% read raw velocity kalman filter output data
fid = fopen("velkf_raw.txt");
data = textscan(fid, '%4xs16');
fclose(fid);
data = data{1,1};
data = data';
...
% calculate rotated gps velocity and velocity error
for i = 1:count
    % check if the velocity sample is not empty
    if not(isnan(vel(:,i)))
        % valid velocity sample
        vel_sample(i) = true;
        % calculate rotated gps velocity
        vel_NED = vel(:,i);
